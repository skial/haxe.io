...
relavant GitHub page. This is to encourge community contribution and make it easy to
...
connect app which targets Adobe AIR, displaying live streams, realtime comments, votes and 
...
Flambe has had amazing growth and adoption with over 200 commerical games built 
with Flambe. Bruno has a strong focus on developer UX, focused on tool intergration
...
ontop of NodeJS and Webkit, it is free and open source and integrates with Haxelib. HIDE is under
...
one framework or finished, that it is a continusous work in progress of many small
...
optmized loops with x1.5 to x4 times speed up for Flash and faster on other targets,
...
UTF8 amoung other accomplishments, Go's objectives are to be readable, safe and
efficeient, provide good concurreny and a tool which can operate at Google's scale.
...
loves to create things often with their own hands, often electronics”_, respectivly.
...
develops open source industrial machines made for a fraction of the commerical costs.
If you've never heard of it definitly check it out.
...
Stereolithography, Powder Deposition, Direct Metal Laser Sintering and Subractive 
...
maker space, check out his [demo][l36] for his take on an in browser 3D editor. The 
...
targets with plans to add Flambe and OpenFL as targets for his experiements but
...
Make sure to head over to his [google+ post][l40] for a more indepth explantion of his
talk.
...
awesome editor, its a place for Flash refugees, it has an asset store and you shouldnt
...
Andreas takes you through designing a restuarant system using, inheritance, interfaces
...
## Joshua Granick - OpenFL 2.0 What was, What is and Whats to come

[Joshua][tw13] wasnt able to attend the conference in person but contributed to it by
...
basiclly ever feature apart from ActionScript bytecode or any sound. The SWF library
creates two formats, `swf` and `swflite`. The `swflite` format is created at compile
time where as the `swf` format is processed during runtime which is inefficent 
compared to the `swflite` format and inpracticle for the HTML5 target. The SWF library
...
in the future, and with Apple annoucning support for WebGL there is nothing holding
...
targets. Unfortunantly but understandably, this will be a paid option to get access
...
perform move advanced actions to assets changes.
