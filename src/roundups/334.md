...
[François Benjamin][tw21] has written an [introduction to Haxe][l40][fr], showing
a couple code examples and the ethusiasum of someone whos just come across Haxe.
...
compiler targets. `BigInt` is located in the [thx.core][l35] library. `BigInt`
allows arithmetic operations on itegers of _unlimited_ size, unless your
short on memory and time.
...
[Haxe and JavaScript][l37] together.

When you've gotten yourself aquainted with Haxe's JavaScript target, maybe checkout
[Philippe Elsass][tw21] CreateJS type definition [generator tool][l38] for Adobe
Flash CC, which was recently updated.

And to finish off this weeks roundup, an article from [Jason Sturges][tw11] on
